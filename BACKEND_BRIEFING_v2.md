...
### Priorität 1.5: Manueller Spielstart nach Vorbehalt 🔴

**Hintergrund:** Aktuell beginnt das Spiel sofort nach Klärung des Vorbehalts.
Der Mensch hat keine Möglichkeit, seine Hand erst in Ruhe zu betrachten oder
zu signalisieren „ich bin bereit". Das wirkt unnatürlich.

**Erwartetes Verhalten:**

Zwischen `phase: 'finding'` / `phase: 'finding-vorbehalt-type'` und
`phase: 'playing'` wird eine neue Wartephase eingeschoben:

```
phase: 'finding' → (alle 'gesund')
   ODER
phase: 'finding-vorbehalt-type' → (Typ gewählt)
        ↓
phase: 'ready-to-play'          ← NEU
        ↓
   Backend wartet auf game:start-playing vom Menschen
        ↓
phase: 'playing'
   Erste Karte wird gespielt
```
...
**Verhalten:**
- Nur der Mensch sendet diesen Event (auch wenn KI ausspielt)
- KIs spielen ihre erste Karte erst NACH `game:start-playing` vom Menschen
- Wenn der Event von einem ungültigen Sender (KI-Simulation) käme:
  Backend ignoriert oder schickt `game:error`
...
type GamePhase =
  ...
  | 'playing'
  | 'finished'
  | 'round-finished'           // ← NEU (siehe Priorität 2.5)
```

**State im `ready-to-play`:**
- `hand` ist gesetzt (sortiert)
- `currentPlayerId` zeigt schon auf den Ausspielspieler
- `currentTrick` ist leer
- KIs warten — keine Auto-Moves bis `game:start-playing` ankommt
